-- Gold layer -- Spark SQL (primary)
--
-- Spark SQL is the PRIMARY implementation for all three analytical
-- questions; PySpark equivalents live in 03_gold_pyspark_alt.py (also wired
-- into the pipeline, as *_pyspark_alt tables) so both are actually executed
-- every run and can be diffed against each other -- see
-- 04_gold_verification.py.
--
-- Why SQL as primary: these three questions are set-based aggregations and
-- joins (GROUP BY, window function, a left join) that read most naturally as
-- SQL, and materialized views let each Gold table declare its own
-- freshness/quality contract right next to the query. PySpark earns its keep
-- upstream in Bronze/Silver, where we need imperative control (retries, a
-- dynamic per-file table factory, defensive column-name lookups for the
-- population API).
--
-- Table/schema names below are hardcoded (gold., silver.) rather than
-- parameterized -- unlike the Python pipeline files, this plain SQL source
-- has no access to the pipeline's spark.conf values. If you rename the
-- silver/gold schemas, update the references in this file to match.
--
-- Plain .sql source file (not a notebook) -- see the header comment in
-- 01_bronze.py for why.

-- ============================================================================
-- Q1 -- mean & stddev of annual US population, 2013-2018 inclusive
--
-- STDDEV() in Spark SQL is the sample standard deviation (STDDEV_SAMP,
-- Bessel-corrected, dividing by N-1) -- the conventional default when
-- treating these six years as a sample. STDDEV_POP is included alongside it
...
--
-- "Quarters" = periods Q01-Q04; Q05 (BLS's annual-average marker) is
-- excluded so it isn't double-counted against the four real quarters. Ties
-- (two years with an identical summed value) are broken by taking the
-- earlier year, for a deterministic result -- documented here since the spec
-- doesn't say.
-- ============================================================================
...
-- ============================================================================
-- Q3 -- PRS30006032, period Q01: value by year, joined with that year's population
--
-- A LEFT JOIN, per the spec ("joined... where available") -- population is
-- NULL for any year the DataUSA API doesn't cover.
-- ============================================================================
