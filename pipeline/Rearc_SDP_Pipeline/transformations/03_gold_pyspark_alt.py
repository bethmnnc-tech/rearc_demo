"""
Gold layer -- PySpark (documented alternate).

Same three questions as 03_gold_sql.sql, implemented with the PySpark
DataFrame API instead of Spark SQL. These are wired into the pipeline as
real tables (not just left in a notebook) so they actually run every update
and can be diffed against the SQL primary in 04_gold_verification.py --
"equivalent alternate" here means "provably produces the same answer," not
just "looks similar."
...
@dp.expect_or_fail("has_all_six_years", "years_included = 6")
def gold_population_stats_pyspark_alt():
    df = spark.read.table(f"{SILVER_SCHEMA}.silver_population").filter(F.col("year").between(2013, 2018))
    return df.select(
...
@dp.expect_or_drop("has_best_year", "best_year IS NOT NULL")
def gold_bls_best_year_per_series_pyspark_alt():
    quarterly = spark.read.table(f"{SILVER_SCHEMA}.silver_bls_data").filter(
    ...
    series = spark.read.table(f"{SILVER_SCHEMA}.silver_bls_series").select("series_id", "series_label")
...
def gold_prs30006032_q01_population_pyspark_alt():
    data = spark.read.table(f"{SILVER_SCHEMA}.silver_bls_data").filter(
        (F.col("series_id") == "PRS30006032") & (F.col("period") == "Q01")
    )
    population = spark.read.table(f"{SILVER_SCHEMA}.silver_population")
